...
function relatedRooms(room, limit = 8) {
  const current = room && room.slug;
  const group = room && room.group;
  const same = rooms.filter((item) => item.slug !== current && item.group === group);
  const extra = rooms.filter((item) => item.slug !== current && !same.includes(item));

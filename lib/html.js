...
const { pageTitle, absoluteUrl, jsonLd, breadcrumb, webmasterTags, homeFaqs, SITE_NAME } = require("./seo");
...
function layout({ title, description, keywords, path, body, noindex, type, crumbs, faqs, items }) {
  ...
  const schema = JSON.stringify(
    jsonLd({ base: siteUrl, path, title: fullTitle, description: desc, type, crumbs, faqs, items })
  );
  const bot = noindex ? "noindex, nofollow" : "index, follow";
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<meta name="googlebot" content="${bot}" />
<meta name="bingbot" content="${bot}" />
...
<link rel="alternate" type="application/rss+xml" title="${SITE_NAME} rooms" href="/feed.xml" />
...
<meta property="og:image:alt" content="${escapeHtml(SITE_NAME)} — free online chat rooms" />
...
function relatedList(room) {
  const list = relatedRooms(room);
  if (!list.length) return "";
  return `<section class="related">
    <h2>Related chat rooms</h2>
    <ul class="local-list">${list.map((item) => `<li><a href="${item.href}">${escapeHtml(item.name)} chat room</a></li>`).join("")}</ul>
  </section>`;
}
...
function homePage() {
  const faqs = homeFaqs();
  const popular = popularPills.map((slug) => getRoom(slug)).filter(Boolean);
  return layout({
    ...
    body: `
      <section class="hero">
        <div class="hero-copy">
          ...
          <h1>Free online chat rooms</h1>
          <p class="hero-lead">Type a name, pick a room, and talk live. No account. Everyone in the same room sees the same chat.</p>
        </div>
        ${startForm("tamil")}
      </section>
      ${roomFinder()}
...
function regionPage(region) {
  ...
  return layout({
    ...
    crumbs,
    items: regionRooms,
    keywords: `${region.title}, free chat rooms, online chat`,
    body: `
      ${breadcrumb(crumbs)}
      <div class="page-head">
      ...
      <div class="room-pick">
        ${regionRooms
          .map(
            (room) =>
              `<a class="room-card" href="${room.href}"><b>${escapeHtml(room.name)}</b><span>${escapeHtml(room.name)} chat room</span></a>`
          )
          .join("")}
      </div>
    `
...
function roomPage(room) {
  const crumbs = [
    { href: "/", label: "Home" },
    { href: "/chat-rooms/", label: "Chat rooms" },
    { href: room.href, label: `${room.name} chat` }
  ];
  return layout({
    title: `${room.name} chat room`,
    description: `Free ${room.name} online chat room. No registration. Talk live with people in ${room.name} and tap a name to chat privately.`,
    path: room.href,
    keywords: `${room.name} chat, ${room.name} chat room, free ${room.name} online chat, ${room.name} chatting`,
    type: "WebPage",
    crumbs,
    body: `
      ${breadcrumb(crumbs)}
      <div class="page-head">
        ...
        <p>Join the free <strong>${escapeHtml(room.name)}</strong> online chat room. No signup. Everyone in this public room can see the conversation. Tap a name to talk privately.</p>
      </div>
...
function lobbyPage() {
  ...
  return layout({
    ...
    body: `
      ${breadcrumb(crumbs)}
      <div class="page-head">
        <h1>Start live chat</h1>
        <p>No registration. Enter a name and talk with everyone in the same room.</p>
      </div>
      ${chatKicker("General")}
  ...
  return layout({
    ...
    body: `
      ...
      <div class="page-head">
        ...
        <p>Search or tap a city, language, or country room. No registration.</p>
      </div>
      ${roomFinder()}
    `
  });
